function renew_api() {
    swal({
        ...
        text: t("The current key will be revoked and cannot be used anymore"),
        icon: "warning",
        buttons: true,
        dangerMode: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: t('Go for it')
    })
        .then((willDelete) => {
            if (willDelete) {
...
function save_user_password() {
    ...
    if ( $('#user_password').val() != $('#user_password_v').val()) {
        ...
        $('#user_password').after("<div class='invalid-feedback' id='user_password-invalid-msg'>" + t('Password and verification are not the same') + "</div>");
        $('#user_password').show();
        return False;
    }
...
function refresh_user_permissions() {
    ...
    $('#user_refresh_perms_btn').text(t('Refreshing..'));
...
$('input[type=radio][name=user-language]').change(function() {
    if (!this.value) {
        return;
    }
    const lang = this.value;
    get_request_api('language/set/' + lang)
        .then((data) => {
            if (notify_auto_api(data, true)) {
                location.reload(true);
            }
